...
def loss(input):
	...
	output=sqsum(diff)
	return output
def stoloss(input,rand):
	y_pred=x_samp_@input
	diff=df.iloc[rand,0]-y_pred[rand,0]
	output=diff**2
	return output
...
def grad(input,rand):
	...
	for i in range(3):
		input_[i,0]+=d
		output[i,0]=(stoloss(input_,rand)-stoloss(input,rand))/d
		input_[i,0]-=d
...
for i in range(iter):
	...
	step=grad(beta,rand)
	beta-=step
	if(loss(beta)<best_stoloss):
		best_no=i+1
		best=np.copy(beta)
		best_stoloss=loss(best)
	if(i>=iter-final_no):
		final[i-iter+final_no]=loss(beta)
print("Smallest stoloss=",best_stoloss)
